...
MergeCSFiles <- function(earlyData,lateData,earlyTag,lateTag){
  
  earlyDataKey <- na.omit(earlyData[!is.na(motheridno),c("motheridno",sprintf("%seventnum",earlyTag),
                                                         "minDate","maxDate"),with=F])
  lateDataKey <- na.omit(lateData[!is.na(motheridno),
                                  c("motheridno",sprintf("%seventnum",lateTag),"minDate","maxDate"),with=F])
